...
module Guillotine
  module StatementExecutors
    describe SingleExecutor do
      before(:each) do
        @parser = mock 'parser'
        @pre_parser = mock 'pre parser'
        @executor = SingleExecutor.new(@pre_parser, @parser)
      end
      ...
      describe "with invalid sql" do
        ...
        it "should raise a SQLParseError if it cannot parse the query with the parser" do
          lambda { 
            @executor.parse("some sql")
          }.should raise_error(Parser::SQLParseError)
        end
        
        it "should include the query in the error" do
          lambda { 
            @executor.parse("some sql")
          }.should raise_error(Parser::SQLParseError, "Could not parse query: some sql")
        end
      end
